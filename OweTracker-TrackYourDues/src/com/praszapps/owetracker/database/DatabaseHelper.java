...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	// Environment.getExternalStorageDirectory()+"/owetrackerDB";
	...
	public static final String SQL_CREATE_TABLE_FRIEND = "CREATE TABLE IF NOT EXISTS "
			+ TABLE_FRIEND
			+ "("
	...
	public static final String SQL_CREATE_TABLE_DUE = "CREATE TABLE IF NOT EXISTS "
			+ TABLE_DUE
			+ "("
	 ...
	 * 
	 * @param friend
	 *            - object of the Friend class to be added to the database
	 * @param db
	 *            - Database instance
	...
	/**
	 * Used to retrieve details of all the friends that are stored in the
	 * database, sorted by their names
	 * 
	 * @param db
	...
	public static ArrayList<Friend> getAllFriendRecords(SQLiteDatabase db) {
		ArrayList<Friend> friendDataList = new ArrayList<Friend>();
		// Select All Query
		String selectQuery = "SELECT * FROM friend ORDER BY friend_name";
		...
		if (cursor.moveToFirst()) {
			do {
				...
				friend.setName(cursor.getString(1));
				friend.setCurrency(cursor.getString(2));
				friend.setAmount(cursor.getInt(3));
	 ...
	 * 
	 * @param friend
	 *            - The friend object to be updated
	 * @param db
	 *            - Database instance
	...
	public static Boolean updateFriend(Friend friend, SQLiteDatabase db) {
		String sql = String.format(
				"SELECT friend_id FROM friend WHERE friend_id = '%s'",
				friend.getId());
	...
	/**
	 * Method updates the due value of an existing friend, called after
	 * adding/editing/removing a due of a particular friend
	 * 
	 * @param friendId
	 *            - The ID of the friend whose total dues need to be updated
	 * @param db
	 *            - Database instance
	 */
	public static void updateFriendDue(String friendId, SQLiteDatabase db) {
		...
		sql = String.format("SELECT SUM(" + DUE_COLUMN_AMOUNT
				+ ") AS TOTAL_DUE FROM " + TABLE_DUE + " WHERE "
				+ DUE_COLUMN_FRIEND_ID + " = '%s'", friendId);
		...
		sql = String.format(
				"SELECT friend_name FROM friend WHERE friend_id = '%s' ;",
				friendId);
		c = db.rawQuery(sql, null);
		if (c.getCount() > 0) {
			c.moveToFirst();
	 ...
	 * 
	 * @param friendId
	 *            - Unique ID of the friend whose currency is sought
	 * @param db
	 *            - Database instance
	 * @return Currency symbol
	 */
	public static String getCurrency(String friendId, SQLiteDatabase db) {
		String sql = "SELECT currency FROM " + TABLE_FRIEND + " WHERE "
				+ FRIEND_COLUMN_ID + " = '" + friendId + "';";
	 ...
	 * 
	 * @param friendId
	 *            - Friend whose data needs to be retrieved
	 * @param db
	 *            - Database instance
	...
	public static Friend getFriendData(String friendId, SQLiteDatabase db) {
		String sql = String.format(
				"SELECT * FROM friend WHERE friend_id = '%s'", friendId);
	 ...
	 * 
	 * @param dueId
	 *            - ID of the due to delete
	 * @param db
	 *            - Database instance
	 * @return true if success, false if failure
	 */
	public static Boolean deleteDueData(String dueId, SQLiteDatabase db) {
		String sql = String.format("SELECT " + DUE_COLUMN_ID + ","
				+ DUE_COLUMN_FRIEND_ID + " " + "FROM " + TABLE_DUE + " WHERE "
				+ DUE_COLUMN_ID + " = '" + dueId + "';");
		Cursor c = db.rawQuery(sql, null);
		if (c.getCount() == 1) {
			c.moveToFirst();
			...
			db.delete(TABLE_DUE, whereClause, null);
			updateFriendDue(
					c.getString(c.getColumnIndex(DUE_COLUMN_FRIEND_ID)), db);
			if (!c.isClosed()) {
				c.close();
			}
			return true;
	...
	public static Boolean deleteAllFriendDues(String friendId, SQLiteDatabase db) {
		String sql = String.format(
				"SELECT due_id FROM friend_due WHERE friend_id = '%s' ;",
				friendId);
		Cursor c = db.rawQuery(sql, null);
		if (c.getCount() > 0) {
			c.moveToFirst();
	 ...
	 * 
	 * @param dueToAdd
	 *            - Due object to insert into the database
	 * @param db
	 *            - Database instance
	...
	public static Boolean updateDue(Due dueToUpdate, SQLiteDatabase db) {
		String sql = String.format("SELECT friend_id FROM " + TABLE_DUE
				+ " WHERE " + DUE_COLUMN_ID + " = '%s' ;",
				dueToUpdate.getDueId());
		Cursor c = db.rawQuery(sql, null);
		if (c.getCount() == 1) {
			c.moveToFirst();
			ContentValues values = new ContentValues();
			...
			String whereClause = DUE_COLUMN_ID + " = '"
					+ dueToUpdate.getDueId() + "'";
			if (!c.isClosed()) {
				c.close();
			}
	 ...
	 * 
	 * @param friendId
	 *            - The friendID of the friend
	 * @param db
	 *            - Database instance
	 * @return - ArrayList of Due type, the list of Dues for the given friend
	 */
	public static ArrayList<Due> getFriendDueList(String friendId,
			SQLiteDatabase db) {
		ArrayList<Due> dueList = new ArrayList<Due>();
		// Select All Query
		String selectQuery = "SELECT * FROM " + TABLE_DUE + " WHERE "
				+ FRIEND_COLUMN_ID + " = '" + friendId + "' ORDER BY "
				+ DUE_COLUMN_DATE + " DESC;";
		...
		if (cursor.moveToFirst()) {
			do {
				Due due = new Due();
				due.setDueId(cursor.getString(cursor
						.getColumnIndex(DUE_COLUMN_ID)));
				due.setFriendId(cursor.getString(cursor
						.getColumnIndex(DUE_COLUMN_FRIEND_ID)));
				due.setDate(Long.parseLong(cursor.getString(cursor
						.getColumnIndex(DUE_COLUMN_DATE))));
				due.setAmount(Integer.parseInt(cursor.getString(cursor
						.getColumnIndex(DUE_COLUMN_AMOUNT))));
				due.setReason(cursor.getString(cursor
						.getColumnIndex(DUE_COLUMN_REASON)));
				// Adding friend to list
				dueList.add(due);
			} while (cursor.moveToNext());
		}
	...
	public static ArrayList<Due> getAllDueRecords(SQLiteDatabase db) {
		ArrayList<Due> dueList = new ArrayList<Due>();
		// Select All Query
...
*/
	...
	private static String lastDueAdded(String friendID, SQLiteDatabase db) {
		Cursor c = db.query(TABLE_DUE, new String[] { "MAX(" + DUE_COLUMN_DATE
				+ ")" }, DUE_COLUMN_FRIEND_ID + " = '" + friendID + "'", null,
				null, null, null);
		c.moveToFirst();
